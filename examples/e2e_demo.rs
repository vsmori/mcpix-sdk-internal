...
fn main() {
    ...
    receiver_bank
        .register_seed(&seed_id, seed_for_bank)
        .unwrap();
    ...
    step(
        2,
        "recebedor (offline) gera par atômico (C₁, C₂) e expõe instrumento",
    );
    ...
    let retained = receiver_sdk
        ...
        .unwrap();
    println!(
        "• campo de transporte (público): {}",
        charge.transport_field
    );
    println!(
        "  └─ tamanho: {} chars (alvo 26..=35)",
        charge.transport_field.len()
    );
    println!(
        "• C₂ retido localmente (secreto): {}",
    ...
    step(
        3,
        "banco do pagador identifica protocolo, recompõe C₂ via lookup",
    );
    ...
    step(
        4,
        "recebedor (ainda offline) compara C₂ apresentado em tempo constante",
    );
    ...
    receiver_bank
        .register_seed(&q_sid, q_seed_for_bank)
        .unwrap();
    ...
    step(
        8,
        "avança o relógio +30s → próximo quantum → 2ª cobrança OK",
    );
    clock.advance(30);
    ...
    step(
        9,
        "banco pagador processa com tolerância de ±1 janela (drift simulado)",
    );
    ...
    let receipt_w = payer_bank_q
        ...
        .unwrap();
    println!(
        "• candidatos emitidos pelo banco: {}",
        receipt_w.candidates.len()
    );
    for (t, c2) in &receipt_w.candidates {
        let mark = if *t == q_charge.counter {
            "← match esperado"
        } else {
            ""
        };
    ...
    step(
        10,
        "recebedor tenta cada candidato — primeiro match é o vencedor",
    );
    let mut accepted = false;
    for (_, c2) in &receipt_w.candidates {
        if matches!(
            q_sdk
                .validate_receipt(&q_sid, q_charge.counter, c2)
                .unwrap(),
    ...
    assert!(
        accepted,
        "windowed flow should accept the matching candidate"
    );
